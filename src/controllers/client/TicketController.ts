...
export default class TicketController implements GenerateReport {
  ...
  public buyTicket(
    ...
    try {
      ...
      switch (type) {
        case 'bus':
          transport = new BusTicket(owner, origin, destiny, seatOption, 'bus', seatType, email)
          break
        case 'plane':
          transport = new PlaneTicket(owner, origin, destiny, seatOption, 'plane', seatType, email)
          break
        default:
